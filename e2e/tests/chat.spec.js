...
test.describe.serial("채팅 E2E 테스트", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // 사용자 데이터 생성
    testUsers = Array.from({ length: USER_COUNT }, (_, i) =>
      createTestUser(i + 1, testId)
    );
  ...
  test.describe("채팅방 관리", () => {
    test("새 채팅방 만들기", async ({ page }) => {
      const roomName = `테스트_채팅방_${Math.random()
        .toString(36)
        .substring(2, 8)}`;
  ...
  test.describe("메시지 전송", () => {
    ...
    test("금칙어 포함 메시지 전송 시 에러 토스트 표시", async ({ page }) => {
      ...
      const sentMessage = page
        ...
        .filter({ hasText: forbiddenMessage });
      await expect(sentMessage).not.toBeVisible();
    });

    test("일반 텍스트 메시지 전송", async ({ page }) => {
      const message = `안녕하세요! 테스트 메시지입니다. ${Math.random()
        .toString(36)
        .substring(7)}`;
    ...
    test("이미지 파일 업로드 및 검증", async ({ page }) => {
      ...
      const message = `이미지 파일 업로드 테스트 ${Math.random()
        .toString(36)
        .substring(7)}`;
      ...
      // 6. 이미지 요소가 존재하는지 확인
      const imageElement =
        fileMessageContainer.getByTestId("file-image-preview");
      ...
      // 8. 파일명이 표시되는지 확인
      await expect(
        fileMessageContainer.locator("text=/profile\\.jpg/i")
      ).toBeVisible();

      // 9. 파일 크기가 표시되는지 확인 (KB, MB 등)
      await expect(
        fileMessageContainer.locator("text=/\\d+(\\.\\d+)?\\s*(KB|MB|GB)/i")
      ).toBeVisible();

      // 10. FileActions 버튼들이 렌더링되는지 확인
      await expect(
        fileMessageContainer.getByTestId("file-download-button")
      ).toBeVisible();
      await expect(
        fileMessageContainer.getByTestId("file-view-button")
      ).toBeVisible();
    });

    test("PDF 파일 업로드 및 검증", async ({ page }) => {
      const filePath = path.resolve(__dirname, "../fixtures/pdf/sample.pdf");
      const message = `PDF 파일 업로드 테스트 ${Math.random()
        .toString(36)
        .substring(7)}`;
      ...
      // 6. 파일명이 표시되는지 확인
      await expect(
        fileMessageContainer.locator("text=/sample\\.pdf/i")
      ).toBeVisible();

      // 7. 파일 크기가 표시되는지 확인
      await expect(
        fileMessageContainer.locator("text=/\\d+(\\.\\d+)?\\s*(B|KB|MB|GB)/i")
      ).toBeVisible();
  ...
  test.describe("실시간 채팅", () => {
    test("다자간(5인) 실시간 메시지 송수신", async ({ browser }) => {
      ...
      await expect(
        hostPage.getByTestId("chat-messages-container")
      ).toBeVisible();
  ...
  test.describe("메시지 읽음 상태", () => {
    test("2인 채팅에서 상대방이 메시지 확인 시 모두 읽음 상태로 변경", async ({
      browser,
    }) => {
      // 1. user1: 채팅방 생성
      ...
      await expect(
        user2Page.getByTestId("chat-messages-container")
      ).toBeVisible();
      ...
      const user2MessageContainer = user2Page
      ...
      // 5. user1 화면: "모두 읽음" 확인
      const user1MessageContainer = user1Page
        .getByTestId("message-container")
        .filter({ hasText: message });
      await expect(
        user1MessageContainer.getByTestId("read-status-all-read")
  ...
  test.describe("채팅 히스토리", () => {
    test("메시지 전송 후 새로고침하여 히스토리 끝 확인", async ({ page }) => {
      // 신규 채팅방 생성 및 입장
      const roomName = `히스토리_테스트_채팅방_${Math.random()
        .toString(36)
        .substring(2, 8)}`;
      await createChatRoomAction(page, roomName);
